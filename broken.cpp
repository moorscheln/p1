...
private:
    ...
    int partition(elmtype arr[], int lo, int hi, int indexPivot)
    {
        elmtype x = arr[indexPivot];
        swap(array[indexPivot], arr[hi]);
        int i = lo;
        for (int j = lo; j <= hi; j++)
        {
            if (arr[j] < x)
    ...
    // {
    //     srand(time(NULL));
    //     int random = lo + arc4random() % (hi - lo + 1);
    ...
    elmtype kthSmallest(elmtype arr[], int l, int r, int k)
    {
        ...
        {
            return kthSmallest(arr, pivot + 1, r, k - l);
        }
    }
